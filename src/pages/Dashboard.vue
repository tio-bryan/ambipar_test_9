...
async function getData() {
  const res = await axios('https://economia.awesomeapi.com.br/json/daily/USD-BRL/30');
  const data = res.data;

  data.forEach((e) => {
    timestamp.push(timeConverter(e.timestamp));
    high.push(e.high);
    low.push(e.low);
    varBid.push(e.varBid);
    pctChange.push(Number(e.pctChange));
  });
}

await getData()

pctChange.sort(function(a, b){return a - b});
